...
(function () {
  ...
  function renderTradeCashStrip(summary, checkingAction) {
    summary = summary || portfolioSummary();
    return '<div class="v20-trade-cash-strip">' +
      '<span><b>Investment Cash</b><strong data-ledger-money="brokerage-cash">' + money(summary.investmentCash || 0) + '</strong><em>Used by Buy / Buy Max.</em></span>' +
      '<span><b>Checking</b><strong data-ledger-money="checking">' + money(summary.checking || 0) + '</strong><em>' + esc(checkingAction || "Use Buy Checking to spend this.") + '</em></span>' +
      '</div>';
  }
  function renderHero() {
    var s = ensureShape();
  ...
  function renderLiveFocus(stock, detail, summary, inputId) {
    ...
    return '<div class="v20-focus-copy"><span>' + esc(stock.sector) + ' / ' + esc(stock.style) + '</span><p>' + esc(stock.desc) + '</p></div>' +
      ...
      renderTradeCashStrip(summary, "Used by Buy Checking.") +
  ...
  function renderAnnualFocus(stock, detail, summary, inputId) {
    ...
    return '<div class="v20-annual-band">' +
      ...
      renderTradeCashStrip(summary, "Fund Investment Cash before annual buys.") +
  ...
  function renderFocusDesk(stock, mode, list, totalMatches, limitToWatchlist) {
    ...
    return '<section class="v20-panel v20-live-panel"><div class="v20-panel-head"><div><span>STOCKS</span><b>' + (limitToWatchlist ? "Watchlist Desk" : "Trading Desk") + '</b></div><strong data-v20-cycle-label>' + esc(cycleLabel()) + '</strong></div>' +
      ...
      '<div class="v20-focus-meta"><span>Pattern <b data-v20-focus-pattern>' + esc(detail.pattern) + '</b></span><span>Annual model <b data-v20-focus-annual>' + pct(profile.expected) + '</b></span><span>Market float <b data-v20-focus-liquidity>' + money(detail.liquidity || 0) + '</b></span></div>' +
  ...
  function installStyles() {
    ...
    style.textContent = [
      ...
      ".v20-position-strip{display:grid;grid-template-columns:repeat(4,minmax(0,1fr));gap:7px;margin:0 0 10px}.v20-position-strip span,.v20-action-group>b{display:block;border:1px solid rgba(255,255,255,.08);border-radius:10px;background:rgba(255,255,255,.035);padding:8px;color:#a89472;font-family:'JetBrains Mono',monospace;font-size:8px;text-transform:uppercase;letter-spacing:.08em;min-width:0}.v20-position-strip b{display:block;margin-top:3px;color:#f0cf86;font-size:10px;letter-spacing:0;text-transform:none;overflow-wrap:anywhere}.v20-trade-cash-strip{display:grid;grid-template-columns:repeat(2,minmax(0,1fr));gap:7px;margin:0 0 10px}.v20-trade-cash-strip span{display:block;border:1px solid rgba(143,204,214,.20);border-radius:10px;background:rgba(143,204,214,.055);padding:8px;min-width:0}.v20-trade-cash-strip b,.v20-trade-cash-strip em{display:block;color:#a9c7c9;font-family:'JetBrains Mono',monospace;font-size:8px;text-transform:uppercase;letter-spacing:.08em;font-style:normal}.v20-trade-cash-strip strong{display:block;margin-top:3px;color:#e7f8f5;font-family:'JetBrains Mono',monospace;font-size:11px;letter-spacing:0;overflow-wrap:anywhere}.v20-trade-cash-strip em{margin-top:3px;color:#9b8e76;text-transform:none;letter-spacing:0;line-height:1.35}.v20-action-group{display:grid;gap:6px}.v20-action-group>b{background:transparent;border-color:rgba(216,177,110,.18);padding:6px 8px;color:#d8b16e}",
